...
- [o] 컴파일 할 때 컴파일 결과로 생성된 클래스 파일을 별도 디렉토리에 분리할 수 있다.
//javac -d 저장위치 / 컴파일할소스위치
 `src/main/java/Hello2.java`, `$ javac -d bin/main src/main/java/Hello2.java`
- [o] 다른 디렉토리에 있는 클래스 파일을 실행할 수 있다.
//java -cp 실행할 클래스파일 위치 / 실행할 클래스 파일(class파일은 확장자를뺀다)
...
- [?] 이스케이프 문자의 역할과 사용법을 안다. `ex03/Exam7.java`
